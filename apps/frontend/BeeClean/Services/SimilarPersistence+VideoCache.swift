...
extension SimilarPersistence {
    // MARK: - Video Group Cache

    /// Loads cached video groups and converts to SimilarGroupVM array.
    /// **Main-actor version** — for non-main contexts that need to avoid
    /// watchdog timeouts during scene updates, use
    /// `loadCachedVideoGroupsBackground()` instead.
    ///
    /// Returns immediately; PHAsset validation runs in the background. See
    /// `loadCachedPhotos` for the launch-perf rationale.
    @MainActor
    static func loadCachedVideoGroups() async -> [SimilarGroupVM] {
        let context = container.mainContext
        let descriptor = FetchDescriptor<CachedVideoGroup>()
        ...
        Task.detached(priority: .background) {
            ...
            await MainActor.run {
                deleteVideoGroups(ids: staleGroupIds)
            }
        }
    ...
    nonisolated static func loadCachedVideoGroupsBackground() -> [SimilarGroupVM] {
    ...
    /// `saveVideoGroups` on a fresh `ModelContext(container)`.
    nonisolated static func saveVideoGroupsBackground(_ groups: [SimilarGroupVM]) {
        let context = ModelContext(container)
        ...
        for group in groups {
            ...
            context.insert(CachedVideoGroup(
                groupId: group.id,
                bestId: group.bestId,
        ...
        do {
            ...
            print("[SimilarPersistence.VideoCache] background context.save failed: \(error.localizedDescription)")
        }
    }

    /// Saves video groups to the cache. Replaces all existing video groups.
    @MainActor
    static func saveVideoGroups(_ groups: [SimilarGroupVM]) {
        let context = container.mainContext
        ...
        do {
            ...
            print("[SimilarPersistence.VideoCache] context.save failed: \(error.localizedDescription)")
        }
    }

    /// Persists selection state changes for a single video group.
    @MainActor
    static func updateVideoGroupSelections(groupId: UUID, items: [SimilarGroupItem]) {
        let context = container.mainContext
        let id = groupId
        var descriptor = FetchDescriptor<CachedVideoGroup>(
    ...
    /// Removes specific video groups by ID.
    @MainActor
    static func deleteVideoGroups(ids: [UUID]) {
