...
public struct ImgEncoder {
    ...
    static func detachFromIOSurface(_ shapedArray: MLShapedArray<Float32>) throws -> MLMultiArray {
    ...
    init(resourcesAt baseURL: URL) throws {
        // Prefer Bundle resource lookup (handles any bundle nesting) over
        // raw path construction which can miss resources on device.
        let imgEncoderURL: URL
        if let bundleURL = Bundle.main.url(forResource: "ImageEncoder_mobileCLIP_s2", withExtension: "mlmodelc") {
            imgEncoderURL = bundleURL
        } else {
            // Fallback to path construction (works when the resource is at bundle root)
            imgEncoderURL = baseURL.appending(path: "ImageEncoder_mobileCLIP_s2.mlmodelc")
        }

        #if targetEnvironment(simulator)
        let config = MLModelConfiguration()
        config.computeUnits = .cpuOnly
        self.model = try MLModel(contentsOf: imgEncoderURL, configuration: config)
        #else
        // On device, try .all (Neural Engine + GPU + CPU) first, then fall
        // back through less-capable compute unit sets. Some devices have ANE
        // driver bugs that cause .all to throw even though the model is valid.
        self.model = try Self.loadWithFallback(url: imgEncoderURL)
        #endif
    }
    ...
    private static func loadWithFallback(url: URL) throws -> MLModel {
        ...
        for units in unitOptions {
            do {
                ...
                if units != .all {
                    print("[ImgEncoder] Loaded with fallback compute units: \(units.rawValue)")
                }
                return model
            } catch {
                print("[ImgEncoder] Failed with \(units.rawValue): \(error.localizedDescription)")
                lastError = error
            }
        }
        throw lastError ?? ImageEncodingError.predictionError
    }

    public func computeImgEmbedding(img: UIImage) async throws -> MLShapedArray<Float32> {
        let raw = try await self.encode(image: img)
    ...
    public func encodeBatch(images: [UIImage]) throws -> [MLShapedArray<Float32>] {
        ...
        try autoreleasepool {
            ...
            for i in 0..<batchResults.count {
                ...
                embeddings.append(Self.l2Normalize(MLShapedArray<Float32>(converting: multiArray)))
            }
        }
    ...
    /// L2-normalize an embedding so dot product equals cosine similarity.
